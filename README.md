...
`--draft-model bundled` turns on speculative decoding with the draft head the checkpoint ships (Qwen3.5/3.8 packs carry one): a greedy request decoding alone gets up to three tokens per model call, verified against the model's own logits, so the output is what plain decoding would produce. Several requests at once, sampling, and requests with logit bias or penalties decode plainly; `/health.speculative` shows cycles, drafted and accepted tokens.
