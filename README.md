...
- **Mixed-precision KV cache** — Bits per layer, set at conversion. Quantized after the prefill by default, as mlx-lm does; `--kv-prefill quantized` writes it quantized from the first token for profiles measured that way.
