...
class Tower:
    def __init__(self, config: dict, model_path: Path | None, dtype=mx.bfloat16):
        ...
        self.image_token_id = int(config.get("image_token_index") or config.get("image_token_id") or 100352)
        self.dtype = dtype
        self.model = VisionModel(self.config.vision_config)
        self.layerwise_projectors = [
            WindowQFormerDownsampler(self.config) for _ in (self.config.deepstack_layer_map or [])
        ]
    ...
    def load(self, model_path: Path) -> None:
        ...
        raw = {(k[len("model.") :] if k.startswith("model.") else k): v for k, v in raw.items()}
        ...
        for i, proj in enumerate(self.layerwise_projectors):
            proj.load_weights(cast(strip_prefix(raw, f"layerwise_projectors.{i}.")), strict=True)
        for i, proj in enumerate(self.spatial_projectors):
            proj.load_weights(cast(strip_prefix(raw, f"spatial_projectors.{i}.")), strict=True)
        if self.image_newline is not None and "image_newline" in raw:
            self.image_newline = raw["image_newline"].astype(self.dtype)
        mx.eval(self.model.parameters(), *(p.parameters() for p in self.layerwise_projectors), *(p.parameters() for p in self.spatial_projectors))
    ...
    def _pack(self, projected: mx.array, image_size) -> mx.array:
        ...
        side = int((cfg.vision_config.image_size // cfg.vision_config.patch_size) * Fraction(cfg.downsample_rate))
        nph, npw = grid_shape(image_size, cfg.image_grid_pinpoints, cfg.vision_config.image_size)
        ...
        if self.image_newline is not None:
            newline = mx.broadcast_to(self.image_newline[:, None, None], (channels, grid.shape[1], 1))
    ...
    def encode(self, tiles: list[mx.array], image_sizes: list) -> list[Encoded]:
        ...
        for image, size in zip(tiles, image_sizes, strict=True):
            ...
            *_, states = self.model(pv, output_hidden_states=True)
            ...
            for i, (vision_layer, text_layer) in enumerate(cfg.deepstack_layer_map or []):
                selected = states[vision_layer]
                if cfg.vision_feature_select_strategy == "default":
                    selected = selected[:, 1:]
                extras[int(text_layer)] = self._pack(self.layerwise_projectors[i](selected), size)
            if self.spatial_projectors and cfg.spatial_target_layers:
                ...
                for group, text_layer in enumerate(cfg.spatial_target_layers):
                    extras[int(text_layer)] = self._pack(self.spatial_projectors[group](spatial), size)
            n = next(iter(extras.values())).shape[0]
            out.append(Encoded(mx.zeros((n, cfg.text_config.hidden_size), dtype=self.dtype), extras))
        return out
    ...
    def describe(self) -> dict:
        return {
            ...
            "deepstack_layers": [int(t) for _, t in (self.config.deepstack_layer_map or [])],
