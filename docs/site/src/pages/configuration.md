...
| `--max-draft-tokens` | integer | `3` | cap on the drafts verified per cycle; the regulator picks each cycle's depth below it from the acceptance and the cycle cost it measures (default: 3, the depth with the best gain measured on a 27B) |
