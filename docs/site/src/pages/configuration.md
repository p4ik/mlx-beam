...
| `enable_thinking` | boolean, or the words `true` / `false` | Thinking on or off for this call; handed to the template. For a family whose template has no switch (Harmony, Muse) the answer is opened in the prompt instead - unless tools are declared, since a call needs the channel or recipient that opener would skip; the model then keeps the choice. |
