...
- `developer` reaches a template that knows the role (gpt-oss) as sent;
  one that would only write the word into its frame still gets `system`
  - measured once at load, `/health.template.roles.developer`. Before,
  the role was always rewritten to `system`.
...
- Text held back inside a think block because it could have begun the
  ...
  call, a call the block ended in included - and the stream sends what the
  opener released like the whole answer keeps it.
...
- A Hugging Face repo id downloads a package's draft head sidecar
  (`mtp/`): a second pass fetches every file the checkpoint's config,
  manifest and weight index name beyond the default patterns.
